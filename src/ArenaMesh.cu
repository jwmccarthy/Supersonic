...
//     // Read mesh data
//     std::vector<int4>   h_triangles(h_ArenaMesh.numTriangles);
//     std::vector<float4> h_vertices(h_ArenaMesh.numVertices);
//     file.read(reinterpret_cast<char*>(h_triangles.data()), sizeof(int4) * h_ArenaMesh.numTriangles);
//     file.read(reinterpret_cast<char*>(h_vertices.data()), sizeof(float4) * h_ArenaMesh.numVertices);

//     // Move triangle data to device
//     DeviceArray<int4>   d_triangles(h_triangles);
//     DeviceArray<float4> d_vertices(h_vertices);
